...
def predict_batch(input_tensor, encoder, decoder, en_tokenizer, device):
    ...
    with torch.no_grad():
        ...
        # 初始化解码器隐藏状态 (1, batch_size, hidden_dim)
...
def predict(zh_sentence, encoder, decoder, zh_tokenizer, en_tokenizer, device):
    ...
    generated = predict_batch(
        input_tensor, encoder, decoder, en_tokenizer, device
    )
...
def run_predict():
    ...
    while True:
        ...
        result = predict(
            user_input, encoder, decoder, zh_tokenizer, en_tokenizer, device
        )
        print(f'英文：{result}')
